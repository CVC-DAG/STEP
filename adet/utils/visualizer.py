...
class TextVisualizer(Visualizer):
    ...
    def draw_instance_predictions(self, predictions):
        ...
        recs_scores = predictions.rec_scores

        self.overlay_instances(ctrl_pnts, recs, scores, recs_scores)
    ...
    def overlay_instances(self, ctrl_pnts, recs, scores, recs_scores, alpha=0.5):
        color = (0.2, 0.2, 0.5)

        for ctrl_pnt, rec, score, rec_scores in zip(ctrl_pnts, recs, scores, recs_scores):
            ...
            rec_conf = rec_scores.max(dim=1)[0].min()
            text = "{:.2f}, {:.2f}: {}".format(score, rec_conf, text)
            lighter_color = self._change_color_brightness(color, brightness_factor=1.)
            text_pos = polygon[0]
            text_pos[1] -= 25
            horiz_align = "left"
            font_size = int(self._default_font_size*1.2)
