...
from adet.modeling.step.losses import SetCriterion
from adet.modeling.step.matcher import build_matcher
from adet.modeling.step.models import STEP
...
class TransformerDetector(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.criterion = SetCriterion(self.step.num_classes, box_matcher, point_matcher,
                                      weight_dict, enc_losses, dec_losses, self.step.num_ctrl_points,
    ...
    def preprocess_queries(self, batched_inputs):
        queries = [x["instances"][0].queries.to(self.device) if len(x["instances"])
    ...
    def forward(self, batched_inputs):
        ...
        queries = self.preprocess_queries(batched_inputs)
        output = self.step((images, queries))
    ...
    def forward_query(self, batched_inputs, queries):
        """
            TODO: rewrite
        """
        images = self.preprocess_image(batched_inputs)
    ...
    def prepare_targets(self, targets):
        ...
        for targets_per_image in targets:
            ...
            gt_ctrl_points = raw_ctrl_points.reshape(-1, self.step.num_ctrl_points, 2) / torch.as_tensor([w, h], dtype=torch.float, device=self.device)[None, None, :]
