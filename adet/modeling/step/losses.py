...
def sigmoid_focal_loss(inputs, targets, num_inst, alpha: float = 0.25, gamma: float = 2):
    ...
    if loss.ndim == 4:
        return loss.mean((1, 2)).sum() / (num_inst + 1e-6)
    elif loss.ndim == 3:
        return loss.mean(1).sum() / (num_inst + 1e-6)
    else:
...
class SetCriterion(nn.Module):
    ...
    def loss_boxes(self, outputs, targets, indices, num_inst):
        ...
        losses = {}
        losses['loss_bbox'] = loss_bbox.sum() / (num_inst + 1e-6)

        loss_giou = 1 - torch.diag(generalized_box_iou(
            box_cxcywh_to_xyxy(src_boxes),
            box_cxcywh_to_xyxy(target_boxes)))
        losses['loss_giou'] = loss_giou.sum() / (num_inst + 1e-6)
        return losses
    ...
    def loss_texts(self, outputs, targets, indices, num_inst):
        ...
        if len(idx[0]) != 0:
            ...
            return {'loss_texts': torch.tensor(1e-6).to(outputs["pred_texts"].device)}
    ...
    def loss_ctrl_points(self, outputs, targets, indices, num_inst):
        ...
        losses = {'loss_ctrl_points': loss_ctrl_points / (num_inst + 1e-6)}
        return losses
